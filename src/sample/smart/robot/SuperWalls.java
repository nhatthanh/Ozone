...
/**
 * SuperWalls - a sample robot by CrazyBassoonist based on the sample robot
 * Walls by Mathew Nelson and maintained by Flemming N. Larsen Moves around the
 * outer edge with two targeting systems
 */
public class SuperWalls extends AdvancedRobot {
	...
	public void onScannedRobot(ScannedRobotEvent e) {
		...
		if ((getRoundNum() == 0 || LGRating > HGRating) && getRoundNum() != 1) { // In the first round or when linear
																					// gun got more hitting percentage
		...
		} else { // in second round or when the head-on gun got more hitting percentage, use
					// head-on gun.
